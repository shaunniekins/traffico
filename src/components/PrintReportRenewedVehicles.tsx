import { fetchRenewedVehiclePermits } from "@/api/printData";
import Image from "next/image";
import QRCode from "qrcode.react";
import React, { useEffect, useState } from "react";

interface PrintReportRenewedVehiclesProps {
  selectedBarangay: string;
}

const PrintReportRenewedVehicles = React.forwardRef<
  HTMLDivElement,
  PrintReportRenewedVehiclesProps
>((props, ref) => {
  const [data, setData] = useState<any[] | null>(null);

  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchRenewedVehiclePermits(props.selectedBarangay);
      setData(data || []);
...
PrintReportRenewedVehicles.displayName = "PrintReportRenewedVehicles";

export default PrintReportRenewedVehicles;
